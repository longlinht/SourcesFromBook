...
public class MyActivity extends Activity {
	...
	private void findScreenOrientation() {
		/**
		 * Listing 12-3: Finding the screen orientation relative to the natural
		 * orientation
		 */
		...
		case (Surface.ROTATION_0):
									Log.i("tt", "ROTATION_0"   );
			break; // Natural
		case (Surface.ROTATION_90):	Log.i("tt", "ROTATION_90"   );
			break; // On its left side
		case (Surface.ROTATION_180):	Log.i("tt", "ROTATION_180"   );
			break; // Updside down
		case (Surface.ROTATION_270):	Log.i("tt", "ROTATION_270"   );
	...
	private void calculateOrientation() {
		/**
		 * Listing 12-6: Finding the current orientation using the accelerometer
		 * and magnetometer
		 */
	...
	private void calculateRemappedOrientation() {
		...
		/**
		 * Listing 12-7: Remapping the orientation reference frame based on the
	...
	private void deprecatedSensorListener() {
		/**
		 * Listing 12-8: Determining orientation using the deprecated
		 * orientation Sensor
		 */
		SensorManager sm = (SensorManager) getSystemService(Context.SENSOR_SERVICE);
	...
	SensorEventListener myGyroListener = new SensorEventListener() {
		public void onSensorChanged(SensorEvent sensorEvent) {
			if (lastTime != 0) {
				...
				angle[2] += sensorEvent.values[2] * dT;	Log.i("tt", "onSensorChanged"   );
			}
	...
	private void calculatingAltitude() {
		/**
		 * Listing 12-10: Finding the current altitude using the barometer
		 * Sensor
		 */
